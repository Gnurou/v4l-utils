...
void vidout_usage(void)
{
	printf("\nVideo Output Formats options:\n"
	       ...
	       "                     The bytesperline and sizeimage options can be used multiple\n"
	       "                     times, once for each plane.\n"
...
void vidout_cmd(int ch, char *optarg)
{
	...
	case OptTryVideoOutFormat:
		set_fmts_out = parse_fmt(optarg, width, height, pixfmt, field,
				colorspace, xfer_func, ycbcr, quantization, flags, bytesperline, sizeimage);
		if (!set_fmts_out) {
			vidcap_usage();
			exit(1);
		}
